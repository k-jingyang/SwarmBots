...
void myFunction(int targetX, int targetY, Motor* motorValue, bool* aligned){
  Position currentPos = *getRobotPosition();
  //first quadrant
  uint8_t deltaX = abs(targetX - currentPos.x);
  uint8_t deltaY = abs(targetY - currentPos.y);
  float alpha = atan2f(deltaY, deltaX);
  static float targetAngle = 0.0f;
  if(targetX > currentPos.x && targetY > currentPos.y){
    targetAngle = alpha;
  }
  //second quadrant
  else if(targetX < currentPos.x && targetY > currentPos.y){
    targetAngle = 90.0f+alpha;  
  }
  //third quadrant
  else if(targetX < currentPos.x && targetY < currentPos.y){
    targetAngle = (alpha - 180);
  }
  //fourth quadrant
  else if(targetX > currentPos.x && targetY < currentPos.y){
    targetAngle = (-alpha);
